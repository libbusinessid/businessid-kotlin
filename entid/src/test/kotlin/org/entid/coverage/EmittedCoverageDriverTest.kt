...
class EmittedCoverageDriverTest {
    ...
    private fun cases(): List<Case> {
        val root = File(requireNotNull(System.getProperty("entid.spec.dir")))
        val bytes = File(root, "entid-conformance.binpb").readBytes()
    ...
    fun `every input of the corpus goes through the engine`() {
        ...
        val jsonl = File(System.getProperty("entid.spec.dir"), "entid-conformance.jsonl")
            .readLines()
    ...
    fun `this driver decodes no field that could carry an expectation`() {
        val source = File("src/test/kotlin/org/entid/coverage/EmittedCoverageDriverTest.kt")
