...
//
// `-Pentid.toolchain=N` moves it, and that is how `scripts/verify.sh`
...
private val toolchainJdk: Int =
    providers.gradleProperty("entid.toolchain").map(String::toInt).getOrElse(BuildConstants.TOOLCHAIN_JDK)
...
tasks.withType<Test>().configureEach {
    ...
    systemProperty("entid.spec.dir", specDir.asFile.absolutePath)
    systemProperty("entid.project.version", project.version.toString())
    // The published groupId, read by the tests that freeze the coordinates.
    systemProperty("entid.project.group", project.group.toString())
    ...
    systemProperty("entid.rules.version", rulesVersion)
...
tasks.register<Test>("fuzz") {
    group = "verification"
    description = "Fuzzes the targets under org.entid.fuzz with Jazzer."
    testClassesDirs = files(fuzzClasses)
    classpath = files(fuzzClasspath)
    filter {
        includeTestsMatching("org.entid.fuzz.*")
