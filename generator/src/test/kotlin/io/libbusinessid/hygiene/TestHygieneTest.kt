...
package io.libbusinessid.hygiene
...
class TestHygieneTest {
    private val classesDir = File(
        requireNotNull(System.getProperty("businessid.test.classes")) {
            "the test task must set businessid.test.classes"
        },
    )
    ...
    fun `the compiled test classes are where the build says they are`() {
        ...
        assertTrue(testClasses().size >= 5, "found only ${testClasses().size} test classes")
    }
