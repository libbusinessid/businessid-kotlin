...
class ReadmeTest {
    ...
    private val readme: String by lazy {
        File(System.getProperty("entid.spec.dir")).parentFile.resolve("README.md").readText()
    }
    ...
    fun `the README installs the coordinates this build publishes`() {
        val group = requireNotNull(System.getProperty("entid.project.group"))
        val version = requireNotNull(System.getProperty("entid.project.version")).removeSuffix("-SNAPSHOT")
        ...
        assertTrue(
            """implementation("$group:entid:$version")""" in readme,
            "the Gradle snippet names other coordinates than $group:entid:$version",
        )
        assertTrue("<groupId>$group</groupId>" in readme, "the Maven snippet names another groupId")
        assertTrue("<artifactId>entid</artifactId>" in readme, "the Maven snippet names another artifactId")
    ...
    fun `the figures the README states`() {
        ...
        val cases = File(System.getProperty("entid.spec.dir"), "entid-conformance.jsonl")
            .readLines()
            .count { it.isNotBlank() }
