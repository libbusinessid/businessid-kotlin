...
class GeneratedRulesetTest {
    ...
    fun `the version of the ruleset matches rules dot lock`() {
        val lock = File(System.getProperty("businessid.spec.dir")).parentFile.resolve("rules.lock")
            .readLines()
            .mapNotNull { line ->
    ...
    fun `the engine version follows the version Gradle publishes`() {
        val published = requireNotNull(System.getProperty("businessid.project.version"))
        assertEquals(
            published.removeSuffix("-SNAPSHOT"),
            EngineVersion.VALUE,
            "the constant and the published version drifted apart",
        )
        assertEquals(EngineVersion.VALUE, BusinessIdEngine.default().rulesInfo().engineVersion)
    }

    @Test
    fun `the rules version and the engine version are independent`() {
        val info = BusinessIdEngine.default().rulesInfo()
