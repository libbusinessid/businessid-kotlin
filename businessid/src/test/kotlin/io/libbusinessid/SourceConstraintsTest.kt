...
class SourceConstraintsTest {
    private val sources: List<File> by lazy {
        val root = File(System.getProperty("businessid.spec.dir")).parentFile
            .resolve("businessid/src/main/kotlin")
    ...
    fun `no Protobuf type reaches the library`() {
        ...
        assertEquals(
            emptyList<String>(),
            findings(sources, Regex("""\.binpb""")).filterNot { "Generated from businessid-rules.binpb" in it },
        )
    }

    @Test
    fun `every hand written file carries the licence header`() {
        val missing = handWritten.filterNot { it.readText().startsWith("// Copyright The LibBusinessID Authors.") }
